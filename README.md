...
We maintain Debian Bookworm packages for amd64 and arm64, you can install with:

```sh
wget https://github.com/cartesi/machine-emulator/releases/download/latest/cartesi-machine_debian-bookworm-$(dpkg --print-architecture).deb
dpkg -i cartesi-machine_debian-bookworm-$(dpkg --print-architecture).deb
```

#### Ubuntu

We maintain Ubuntu 24.04 packages for amd64 and arm64, you can install with:

```sh
wget https://github.com/cartesi/machine-emulator/releases/download/latest/cartesi-machine_ubuntu-24.04-$(dpkg --print-architecture).deb
dpkg -i cartesi-machine_ubuntu-24.04-$(dpkg --print-architecture).deb
```
...
First, make sure to have all the system requirements, then run the following to build and install the latest release of the machine:

```sh
# clone the latest release of the emulator
git clone --branch latest https://github.com/cartesi/machine-emulator.git
cd machine-emulator

# patch the sources with required generated files
wget https://github.com/cartesi/machine-emulator/releases/download/latest/add-generated-files.diff
...
# install latest guest Linux kernel and guest rootfs images
sudo mkdir -p $INSTALL_PATH/share/cartesi/images
sudo wget -O $INSTALL_PATH/share/cartesi/images/linux.bin https://github.com/cartesi/image-kernel/releases/download/latest/linux.bin
sudo wget -O $INSTALL_PATH/share/cartesi/images/rootfs.ext2 https://github.com/cartesi/machine-emulator-tools/releases/download/latest/rootfs.ext2
```

Note that we also download guest Linux kernel and guest rootfs images to be used with the emulator. They are required to boot the default Ubuntu distribution with `cartesi-machine`.
