...
--  Argument parsing
-- ---------------------------------------------------------------------------
local command = argparse("bench-insns", "Microbenchmark for Cartesi RISC-V emulator")
command:flag("   -q --quiet",         "Only print overall average"):init(false)
command:option("   -i --iterations N", "Iterations per instruction"):default(1024):convert(tonumber)
command:option("   -f --filter PATTERN", "Only benchmark instructions matching PATTERN (Lua pattern)")
command:argument():args("*"):target("names")("   names", "Explicit list of instruction names to benchmark (overrides --filter)")

local opts = command:parse()

local detailed = not opts.quiet
...
local insns = {
    ...
    {name='c.addi',      opcode=0x050d050d--[[c.addi a0, 3]]},
    {name='c.addiw',     opcode=0x250d250d--[[c.addiw a0, 3]]},
    {name='c.add',       opcode=0x952e952e--[[c.add a0, a1]]},
    {name='c.and',       opcode=0x8d6d8d6d--[[c.and a0, a1]]},
    {name="c.ld",        opcode=0x61886188--[[c.ld  a0, 0(a1)]], a1=DATA_ADDR},
    {name="c.sd",        opcode=0xe188e188--[[c.sd  a0, 0(a1)]], a1=DATA_ADDR},
...
local function filter_insns()
  ...
  if opts.names and #opts.names > 0 then
    ...
    for _, insn in ipairs(insns) do
      if name_set[insn.name] then
        table.insert(filtered, insn)
      end
    end
    return filtered
  end
  ...
  if opts.filter then
    local filtered = {}
    for _, insn in ipairs(insns) do
      if insn.name:match(opts.filter) then
        table.insert(filtered, insn)
      end
    end
    return filtered
  ...
  -- No filter — return all instructions
  return insns
end
...
local function bench_insn(machine, insn, iters)
    local unroll_count = 1023
    local execute_count = unroll_count + 1
    if insn.name:match("^c%.") then
        execute_count = unroll_count * 2 + 1
    end
    local jump = 0x00018067 -- jalr zero,0(gp)
    local prog = string.pack("<I4", insn.opcode):rep(unroll_count) .. string.pack("<I4", jump)
    assert(#prog == 4096)
    -- init machine memory
    machine:write_reg("mstatus", machine:read_reg("mstatus") | MSTATUS_FS_DIRTY) -- enable floating-point
    machine:write_reg("pc", TEXT_ADDR)
    machine:write_reg("mtvec", TEXT_ADDR) -- set trap handler (for ecall/ebreak)
    machine:write_reg("x3", TEXT_ADDR) -- gp
    machine:write_reg("x1", 0) -- ra
    machine:write_reg("x10", insn.a0 or 0) -- a0
    machine:write_reg("x11", insn.a1 or 0) -- a1
    machine:write_reg("x12", insn.a2 or 0) -- a2
    machine:write_reg("x13", insn.a3 or 0) -- a3
    machine:write_reg("f10", insn.fa0 or 0) -- fa0
    machine:write_reg("f11", insn.fa1 or 0) -- fa1
    machine:write_reg("f12", insn.fa2 or 0) -- fa2
    machine:write_reg("f13", insn.fa3 or 0) -- fa3
    machine:write_memory(TEXT_ADDR, prog)
    machine:write_memory(DATA_ADDR, string.pack("<I8", insn.data or 0))
    -- warmup
    local cycles = iters * execute_count
    machine:run(machine:read_reg("mcycle") + cycles)
    assert(machine:read_reg("pc") == TEXT_ADDR)
    -- benchmark
    ...
    local ucycles = machine:read_reg("uarch_cycle")
    machine:reset_uarch()
    return mips, ucycles
end
...
local function bench()
    local filtered = filter_insns()

    if #filtered == 0 then
        print("No instructions matched the given filter.")
        return
    end
    ...
    local mips_sum = 0
    local ucycles_sum = 0
    for _, insn in ipairs(filtered) do
        local mips, ucycles = bench_insn(machine, insn, opts.iterations)
        if detailed then
    ...
    print("-- Average instruction speed --")
    print(string.format("%-44s%9.3f MIPS%9.1f ucycles", "(all)", mips_sum / #filtered, ucycles_sum / #filtered))
end
