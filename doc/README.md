...
The CLI separates starting an epoch, finishing it, and saving epoch
files so it can be resumed.
`begin_epoch:true,end_epoch:true,resumable:false` is the default and
processes an entire epoch with intermediate bookkeeping in memory. For a
resumable run, supply `state_hash` and `outputs_frontier` in
`--cmio-advance-state`, and `frontier` in `--mcycle-computation-hash`
when computing an mcycle computation hash. Output proofs across batches
also require `output_hashes` and `output_input_indices`.

For example, use
`--cmio-advance-state=resumable:true,state_hash:epoch-state.bin,outputs_frontier:outputs.json,output_hashes:hashes.bin,output_input_indices:indices.bin`
together with
...
A new resumable epoch requires missing or empty epoch files and refuses
to replace nonempty files. It starts an empty computation frontier and
initializes the outputs Merkle tree from `last_output_proof`, or starts
it empty at genesis. A continuation requires valid existing epoch files
and a machine matching `state_hash`. Retain that machine with store/load
or a remote server, and reuse the same epoch filenames, hash period,
bundle size, and `last_output_proof` across batches. Uarch computation
hashes can read existing epoch files with `begin_epoch:false`, but
cannot use `resumable:true` or modify the files. Use `next_input_offset`
in `--cmio-advance-state` to identify the zero-based epoch offset of the
next input for the machine, including rejected inputs. With
`begin_epoch:true`, `next_input_offset` must be zero. For uarch
computation hashes, use `begin_epoch:false` with a nonzero offset to
start from a later input without saved epoch files. For mcycle
computation hashes, the offset is derived from the computation frontier,
and an explicitly supplied value must agree.

Resumable runs save completed input boundaries even when
`end_epoch:true` requests the entire epoch in one invocation. The final
invocation seals the computation frontier and writes the requested
output proofs. Initialization can leave empty files if a later check
fails. Epoch file writes are not an atomic transaction, so recovery
