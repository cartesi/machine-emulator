...
by committing to a *computation hash*, the root of a Merkle tree whose
leaves are the machine state hashes sampled along the whole computation,
and the dispute walks down the committed trees instead. Claims can be
...
yield, halt, or mcycle overflow no longer advances, so the leaves past a
stop repeat its state hash (the reverted state hash when an input was
rejected) to the end of the input’s span, and the leaves past the last
input repeat the epoch’s final state hash. When two mcycle claims
disagree on a leaf, having agreed on the leaf before it, each side posts
a *uarch claim* that expands the period between the two leaves, one leaf
per uarch transition. Each of its 2<sup>p</sup> instructions contributes
its real uarch cycles, repetitions of the uarch halt state filling its
...
``` text
Mcycle computation hash: 0xc75bbba25fef709d149eeb65a911bd7a7877713f2984164409da12118631b41e
```
...
The CLI separates starting an epoch, finishing it, and saving epoch files so it can be resumed.
`begin_epoch:true,end_epoch:true,resumable:false` is the default and processes an entire epoch with intermediate bookkeeping in memory.
For a resumable run, supply `state_hash` and `outputs_frontier` in `--cmio-advance-state`, and `frontier` in `--mcycle-computation-hash` when computing an mcycle computation hash.
Output proofs across batches also require `output_hashes` and `output_input_indices`.

For example, use `--cmio-advance-state=resumable:true,state_hash:epoch-state.bin,outputs_frontier:outputs.json,output_hashes:hashes.bin,output_input_indices:indices.bin` together with `--mcycle-computation-hash=log2_mcycle_period:10,frontier:computation.json,filename:mch.bin`.
Add the following batch options to `--cmio-advance-state`:

| Invocation | Batch options |
| --- | --- |
| First batch | `begin_epoch:true,end_epoch:false,input_file_index_begin:0,input_file_index_end:1` |
| Intermediate batch | `begin_epoch:false,end_epoch:false,input_file_index_begin:1,input_file_index_end:2` |
| Final batch | `begin_epoch:false,end_epoch:true,input_file_index_begin:2,input_file_index_end:3` |
| Entire epoch with saved epoch files | `begin_epoch:true,end_epoch:true,input_file_index_begin:0,input_file_index_end:3` |

A new resumable epoch requires missing or empty epoch files and refuses to replace nonempty files.
It starts an empty computation frontier and initializes the outputs Merkle tree from `last_output_proof`, or starts it empty at genesis.
A continuation requires valid existing epoch files and a machine matching `state_hash`.
Retain that machine with store/load or a remote server, and reuse the same epoch filenames, hash period, bundle size, and `last_output_proof` across batches.
Uarch computation hashes can read existing epoch files with `begin_epoch:false`, but cannot use `resumable:true` or modify the files.
Use `next_input_offset` in `--cmio-advance-state` to identify the zero-based epoch offset of the machine's next input, including rejected inputs.
With `begin_epoch:true`, `next_input_offset` must be zero.
For uarch computation hashes, use `begin_epoch:false` with a nonzero offset to start from a later input without saved epoch files.
For mcycle computation hashes, the offset is derived from the computation frontier, and an explicitly supplied value must agree.

Resumable runs save completed input boundaries even when `end_epoch:true` requests the entire epoch in one invocation.
The final invocation seals the computation frontier and writes the requested output proofs.
Initialization can leave empty files if a later check fails.
Epoch file writes are not an atomic transaction, so recovery after a process or host crash can require manual repair.
...
without changing the player driver or builders. A disk-backed
...
Claim 0x4f4b4987..., with final state 0xf0fb49ec..., joined (posted by tamperer).
Claim 0x56d98314..., with final state 0x5d4ca486..., joined (posted by fabulist).
Claim 0x923b6eb7..., with final state 0x5d4ca486..., joined (posted by fixed_fabulist).
Claim 0x98e1ce81..., with final state 0xbd6f4f4e..., joined (posted by forger).
Claim 0xc75bbba2..., with final state 0x5d4ca486..., joined (posted by honest).
...
...
A uarch tournament opens over input 2, period 2000, starting from 0xc83fc2fe....
Claim 0x3c15fedb..., with final state 0x4953b2b7..., joined (posted by fabulist).
Claim 0xa9744aa0..., with final state 0xcba0fec7..., joined (posted by fixed_fabulist).
Round 1, match 2.1, at the uarch level: claim 0x3c15fedb... against claim 0xa9744aa0....
Match 2.1: claim 0xa9744aa0... wins.
The uarch winner confirms 0xcba0fec7.... Claim 0x56d98314... is eliminated.
```
...
Height 59: the claims first disagree within leaves [0x0, 0x7ffffffffffffff].
...
A uarch tournament opens over input 2, period 0, starting from 0xf4018067....
Claim 0xbc1636f3..., with final state 0xf8db3f42..., joined (posted by forger).
Claim 0xfa41d4e9..., with final state 0x87004b64..., joined (posted by honest).
Round 1, match 3.1, at the uarch level: claim 0xbc1636f3... against claim 0xfa41d4e9....
Match 3.1: claim 0xfa41d4e9... wins.
The uarch winner confirms 0x87004b64.... Claim 0x98e1ce81... is eliminated.
```
...
Claim 0x18226d60..., with final state 0x1c5dc695..., joined (posted by tamperer).
Claim 0x31f1a82a..., with final state 0xa04dee27..., joined (posted by fixed_fabulist).
...
Claim 0xace2aca3..., with final state 0x4953b2b7..., joined (posted by fixed_fabulist).
Claim 0x0cd11aa6..., with final state 0x5d4ca486..., joined (posted by honest).
Round 1, match 6.1, at the uarch level: claim 0xace2aca3... against claim 0x0cd11aa6....
...
``` text
Height 1: the claims first disagree within leaves [0x3ffffffe, 0x3fffffff].
The claims diverge at state 1073741823: 0x4953b2b7... against 0x5d4ca486..., from the agreed state 0x9a3c0962....
The disputed transition is a uarch step and the uarch reset closing an instruction.
