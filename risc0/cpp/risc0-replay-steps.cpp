    ...
    interop_hash_type root_hash_after) 
{
    ...
    replay_step_state_access a(context, step_log_image, step_log_image_size, *reinterpret_cast<const machine_hash*>(root_hash_before));
    uint64_t mcycle_end{};
    (void) __builtin_add_overflow(a.read_mcycle(), mcycle_count, &mcycle_end);
    interpret<replay_step_state_access&>(a, mcycle_end);
    a.finish(*reinterpret_cast<const machine_hash*>(root_hash_after));
}
