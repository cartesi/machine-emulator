...
#[no_mangle]
pub extern "C" fn interop_merkle_tree_hash(hash_tree_target: u64, data: *const c_char, size: c_ulong, hash: *mut c_char) {
    if hash_tree_target != 1 {
        panic!("interop_merkle_tree_hash: hash_tree_target must be 1");
    }
    if size > 32 {
        unsafe {
            let half_size = size / 2;
            let left_hash = [0u8; 32];
            interop_merkle_tree_hash(hash_tree_target, data, half_size, left_hash.as_ptr() as *mut c_char);
            let right_hash = [0u8; 32];
            interop_merkle_tree_hash(hash_tree_target, data.add(half_size as usize) as *const c_char, half_size, right_hash.as_ptr() as *mut c_char);
...
#[no_mangle]
pub extern "C" fn interop_concat_hash(hash_tree_target: u64, left: *const c_char, right: *const c_char, result: *mut c_char) {
    if hash_tree_target != 1 {
        panic!("interop_concat_hash: hash_tree_target must be 1");
    }
    let mut conctd = [0u8; 64];
    unsafe {
