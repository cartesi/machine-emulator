...
/// static method implementation.
static int jsonrpc_machine_class_verify_access_log(lua_State *L) {
    ...
    TRY_EXECUTE(cm_jsonrpc_verify_access_log(managed_jsonrpc_mg_mgr.get(), managed_log.get(),
        managed_runtime_config.get(), lua_toboolean(L, 3), err_msg));
    managed_log.reset();
...
/// static method implementation.
static int jsonrpc_machine_class_verify_state_transition(lua_State *L) {
    const int stubidx = lua_upvalueindex(1);
    const int ctxidx = lua_upvalueindex(2);
    lua_settop(L, 5);
    auto &managed_jsonrpc_mg_mgr = clua_check<clua_managed_cm_ptr<cm_jsonrpc_mg_mgr>>(L, stubidx, ctxidx);
    ...
    TRY_EXECUTE(cm_jsonrpc_verify_state_transition(managed_jsonrpc_mg_mgr.get(), &root_hash, managed_log.get(),
        &target_hash, managed_runtime_config.get(), one_based, err_msg));
    managed_log.reset();
    managed_runtime_config.reset();
    lua_pop(L, 2);
...
static const auto jsonrpc_machine_static_methods = cartesi::clua_make_luaL_Reg_array({
    ...
    {"verify_access_log", jsonrpc_machine_class_verify_access_log},
    {"verify_state_transition", jsonrpc_machine_class_verify_state_transition},
