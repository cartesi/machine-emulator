...
/// \brief This is the machine.verify_load_cmio_input_log()
/// static method implementation.
static int jsonrpc_machine_class_verify_load_cmio_input_log(lua_State *L) {
    const int stubidx = lua_upvalueindex(1);
    const int ctxidx = lua_upvalueindex(2);
    lua_settop(L, 4);
    auto &managed_jsonrpc_mg_mgr = clua_check<clua_managed_cm_ptr<cm_jsonrpc_mg_mgr>>(L, stubidx, ctxidx);
    ...
    auto &managed_runtime_config = clua_push_to(L,
        clua_managed_cm_ptr<cm_machine_runtime_config>(clua_opt_cm_machine_runtime_config(L, 4, {}, ctxidx)), ctxidx);
    TRY_EXECUTE(cm_jsonrpc_verify_load_cmio_input_log(managed_jsonrpc_mg_mgr.get(), reason, data, length,
...
/// \brief This is the machine.verify_load_cmio_input_state_transition()
/// static method implementation.
static int jsonrpc_machine_class_verify_load_cmio_input_state_transition(lua_State *L) {
    const int stubidx = lua_upvalueindex(1);
    const int ctxidx = lua_upvalueindex(2);
    lua_settop(L, 6);
    auto &managed_jsonrpc_mg_mgr = clua_check<clua_managed_cm_ptr<cm_jsonrpc_mg_mgr>>(L, stubidx, ctxidx);
    ...
    TRY_EXECUTE(cm_jsonrpc_verify_load_cmio_input_state_transition(managed_jsonrpc_mg_mgr.get(), reason, data, length,
...
static const auto jsonrpc_machine_static_methods = cartesi::clua_make_luaL_Reg_array({
    ...
    {"verify_load_cmio_input_log", jsonrpc_machine_class_verify_load_cmio_input_log},
    {"verify_load_cmio_input_state_transition", jsonrpc_machine_class_verify_load_cmio_input_state_transition},
});
