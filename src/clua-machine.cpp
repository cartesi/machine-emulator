...
/// \brief This is the machine.verify_load_cmio_input_log() method implementation.
static int machine_class_index_verify_load_cmio_input_log(lua_State *L) {
    lua_settop(L, 4);
    ...
    auto &managed_runtime_config =
        clua_push_to(L, clua_managed_cm_ptr<cm_machine_runtime_config>(clua_check_cm_machine_runtime_config(L, 4)));
    TRY_EXECUTE(cm_verify_load_cmio_input_log(reason, data, length, managed_log.get(), managed_runtime_config.get(),
        true, err_msg));
    lua_pushnumber(L, 1);
    managed_runtime_config.reset();
    managed_log.reset();
...
/// \brief This is the machine.verify_load_cmio_input_state_transition() method implementation.
static int machine_class_index_verify_load_cmio_input_state_transition(lua_State *L) {
    lua_settop(L, 6);
    cm_hash root_hash{};
    const uint16_t reason = static_cast<uint16_t>(luaL_checkinteger(L, 1));
    size_t length{0};
    ...
    TRY_EXECUTE(cm_verify_load_cmio_input_state_transition(reason, data, length, &root_hash, managed_log.get(),
...
static const auto machine_class_index = cartesi::clua_make_luaL_Reg_array({
    ...
    {"verify_load_cmio_input_log", machine_class_index_verify_load_cmio_input_log},
    {"verify_load_cmio_input_state_transition", machine_class_index_verify_load_cmio_input_state_transition},
});
