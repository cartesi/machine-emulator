...
static_assert(AR_SHADOW_STATE_START_DEF == CM_AR_SHADOW_STATE_START);
static_assert(AR_SHADOW_STATE_LENGTH_DEF == CM_AR_SHADOW_STATE_LENGTH);
static_assert(AR_SHADOW_UARCH_STATE_START_DEF == CM_AR_SHADOW_UARCH_STATE_START);
static_assert(AR_SHADOW_UARCH_STATE_LENGTH_DEF == CM_AR_SHADOW_UARCH_STATE_LENGTH);
static_assert(AR_UARCH_RAM_START_DEF == CM_AR_UARCH_RAM_START);
static_assert(AR_UARCH_RAM_LENGTH_DEF == CM_AR_UARCH_RAM_LENGTH);
...
static_assert(AR_HTIF_LENGTH_DEF == CM_AR_HTIF_LENGTH);
static_assert(AR_FIRST_VIRTIO_START_DEF == CM_AR_FIRST_VIRTIO_START);
static_assert(AR_LAST_VIRTIO_END_DEF == CM_AR_LAST_VIRTIO_END);
static_assert(AR_PLIC_START_DEF == CM_AR_PLIC_START);
static_assert(AR_PLIC_LENGTH_DEF == CM_AR_PLIC_LENGTH);
static_assert((UINT64_C(1) << AR_CMIO_RX_BUFFER_LOG2_SIZE_DEF) == CM_AR_CMIO_RX_BUFFER_LENGTH);
...
static_assert(cartesi::STEP_LOG_SIGNATURE.size() == CM_STEP_LOG_SIGNATURE_SIZE);
static_assert(std::string_view{cartesi::STEP_LOG_SIGNATURE.data(), cartesi::STEP_LOG_SIGNATURE.size()} ==
    std::string_view{CM_STEP_LOG_SIGNATURE, CM_STEP_LOG_SIGNATURE_SIZE});
...
static_assert(static_cast<uint64_t>(cartesi::hash_function_type::keccak256) == CM_HASH_KECCAK256);
static_assert(static_cast<uint64_t>(cartesi::hash_function_type::sha256) == CM_HASH_SHA256);
...
cm_error cm_verify_step(const cm_hash *root_hash_before, const char *log_filename, uint64_t mcycle_count,
    const cm_hash *root_hash_after, cm_hash *obtained_root_hash) try {
    if (log_filename == nullptr) {
        throw std::invalid_argument("invalid log_filename");
    }
    const cartesi::machine_hash cpp_root_hash_before = convert_from_c(root_hash_before);
    std::optional<cartesi::machine_hash> cpp_root_hash_after;
    if (root_hash_after != nullptr) {
        cpp_root_hash_after = convert_from_c(root_hash_after);
    }
    const cartesi::machine_hash cpp_obtained_root_hash =
        cartesi::machine::verify_step(cpp_root_hash_before, log_filename, mcycle_count, cpp_root_hash_after);
...
cm_error cm_verify_step_uarch(const cm_machine *m, const cm_hash *root_hash_before, const char *log_filename,
    uint64_t uarch_cycle_count, const cm_hash *root_hash_after, cm_hash *obtained_root_hash) try {
    if (log_filename == nullptr) {
        throw std::invalid_argument("invalid log_filename");
    }
    const cartesi::machine_hash cpp_root_hash_before = convert_from_c(root_hash_before);
    ...
    if (m != nullptr) {
        ...
        cpp_obtained_root_hash =
            cpp_m->verify_step_uarch(cpp_root_hash_before, log_filename, uarch_cycle_count, cpp_root_hash_after);
    } else {
        cpp_obtained_root_hash = cartesi::machine::verify_step_uarch(cpp_root_hash_before, log_filename,
            uarch_cycle_count, cpp_root_hash_after);
    }
    if (obtained_root_hash != nullptr) {
        convert_to_c(cpp_obtained_root_hash, obtained_root_hash);
    }
...
cm_error cm_verify_reset_uarch(const cm_machine *m, const cm_hash *root_hash_before, const char *log_filename,
    ...
    if (m != nullptr) {
        const auto *cpp_m = convert_from_c(m);
        cpp_obtained_root_hash = cpp_m->verify_reset_uarch(cpp_root_hash_before, log_filename, cpp_root_hash_after);
    } else {
        cpp_obtained_root_hash =
            cartesi::machine::verify_reset_uarch(cpp_root_hash_before, log_filename, cpp_root_hash_after);
    }
    if (obtained_root_hash != nullptr) {
        convert_to_c(cpp_obtained_root_hash, obtained_root_hash);
    }
...
cm_error cm_log_send_cmio_response(cm_machine *m, const cm_hash *revert_root_hash, uint16_t reason, const uint8_t *data,
    uint64_t length, const char *log_filename) try {
    if (log_filename == nullptr) {
        throw std::invalid_argument("invalid log_filename");
    }
    const cartesi::machine_hash cpp_revert_root_hash = convert_from_c(revert_root_hash);
    auto *cpp_m = convert_from_c(m);
    cpp_m->log_send_cmio_response(cpp_revert_root_hash, reason, data, length, log_filename);
    return cm_result_success();
} catch (...) {
    return cm_result_failure();
}
...
cm_error cm_verify_send_cmio_response(const cm_machine *m, const cm_hash *revert_root_hash, uint16_t reason,
    ...
    if (m != nullptr) {
        const auto *cpp_m = convert_from_c(m);
        cpp_obtained_root_hash = cpp_m->verify_send_cmio_response(cpp_revert_root_hash, reason, data, length,
            cpp_root_hash_before, log_filename, cpp_root_hash_after);
    } else {
        cpp_obtained_root_hash = cartesi::machine::verify_send_cmio_response(cpp_revert_root_hash, reason, data, length,
            cpp_root_hash_before, log_filename, cpp_root_hash_after);
    }
    if (obtained_root_hash != nullptr) {
        convert_to_c(cpp_obtained_root_hash, obtained_root_hash);
