...
complete_merkle_tree::proof_type 
complete_merkle_tree::get_proof(address_type address, int log2_size) const {
...
void
complete_merkle_tree::push_back(const hash_type &hash) {
    auto &leaves = get_level(get_log2_leaf_size());
    if (leaves.size() >=
        address_type{1} << (get_log2_root_size()-get_log2_leaf_size())) {
...
void
complete_merkle_tree::check_log2_sizes(int log2_root_size, int log2_leaf_size,
    int log2_word_size) {
    if (log2_root_size < 0) {
...
const
complete_merkle_tree::hash_type &
complete_merkle_tree::get_node_hash(address_type address, int log2_size) const {
...
void
complete_merkle_tree::bubble_up(void) {
    hasher_type h;
    // Go bottom up, updating hashes
    for (int log2_next_size = get_log2_leaf_size()+1;
        log2_next_size <= get_log2_root_size(); ++log2_next_size) {
        ...
        for (; first_entry < last_safe_entry; ++first_entry) {
            get_concat_hash(h, prev[2*first_entry], prev[2*first_entry+1],
                next[first_entry]);
        }
        // Maybe do last odd entry
        if (prev.size() > 2*last_safe_entry) {
            get_concat_hash(h, prev.back(), m_pristine.get_hash(log2_prev_size),
                next[last_safe_entry]);
        }
    }
}

const complete_merkle_tree::level_type &
complete_merkle_tree::get_level(int log2_size) const {
    if (log2_size < get_log2_leaf_size() ||
        log2_size > get_log2_root_size()) {
        throw std::out_of_range{"log2_size is out of bounds"};
    }
