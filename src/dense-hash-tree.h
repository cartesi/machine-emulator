...
class dense_hash_tree : public i_dense_hash_tree {
    ...
    int check_level_count(int level_count, size_type leaf_count) const {
        if (level_count < 0) {
...
public:
    dense_hash_tree(int level_count, size_type leaf_count) :
        m_level_count{check_level_count(level_count)},
...
private:
    ...
    int to_level(int log2_size) const noexcept {
        return log2_size - AR_LOG2_PAGE_SIZE;
    }

    size_type to_index(uint64_t offset, int log2_size) {
        int level = to_level(log2_size);
        if (level < 0 || level >= m_level_count) {
            return invalid_index;
        }
        size_type start = size_type{1} << (m_level_count - level - 1);
        size_type index = offset >> log2_size;
        if (index >= start) {
            return invalid_index;
        }
        return start + index;
