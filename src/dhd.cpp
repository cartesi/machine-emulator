    ...
    switch (offset) {
        ...
        case dhd_hlength_rel_addr:
            *val = a->read_dhd_hlength();
            return true;
        default:
            if (offset >= dhd_h0_rel_addr && offset < dhd_h0_rel_addr + DHD_H_REG_COUNT * sizeof(uint64_t) &&
                (offset & (sizeof(uint64_t) - 1)) == 0) {
                auto i = static_cast<int>((offset - dhd_h0_rel_addr) / sizeof(uint64_t));
                *val = a->read_dhd_h(i);
                return true;
            }
...
static bool dhd_write_hlength(const pma_entry &pma, i_device_state_access *a, uint64_t hlength) {
    (void) pma;
    ...
    assert((dlength == DHD_NOT_FOUND && data.empty()) || (data.size() == dlength && dlength <= req_dlength));
    if (!(dlength == DHD_NOT_FOUND && data.empty()) && !(data.size() == dlength && dlength <= req_dlength)) {
...
static bool dhd_peek(const pma_entry &pma, const machine &m, uint64_t page_offset, const unsigned char **page_data,
    ...
    // Copy all registers
    aliased_aligned_write<uint64_t>(scratch + dhd_get_csr_rel_addr(dhd_csr::tstart), m.read_dhd_tstart());
    aliased_aligned_write<uint64_t>(scratch + dhd_get_csr_rel_addr(dhd_csr::tlength), m.read_dhd_tlength());
    aliased_aligned_write<uint64_t>(scratch + dhd_get_csr_rel_addr(dhd_csr::dlength), m.read_dhd_dlength());
    aliased_aligned_write<uint64_t>(scratch + dhd_get_csr_rel_addr(dhd_csr::hlength), m.read_dhd_hlength());
    for (int i = 0; i < DHD_H_REG_COUNT; i++) {
        aliased_aligned_write<uint64_t>(scratch + dhd_get_h_rel_addr(i), m.read_dhd_h(i));
    }
    *page_data = scratch;
    return true;
}

static const pma_driver dhd_driver = {"DHD", dhd_read, dhd_write};
...
pma_entry make_dhd_pma_entry(uint64_t start, uint64_t length) {
    ...
    return make_device_pma_entry(start, length, dhd_peek, &dhd_driver).set_flags(f);
}
