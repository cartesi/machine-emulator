...
private:
    ...
    uint64_t do_read_register(int reg) {
        return log_read(m_s->shadow_pma->start + shadow_get_register_rel_addr(reg), m_s->x[reg], "x");
    }

    void do_write_register(int reg, uint64_t val) {
        assert(reg != 0);
        return log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_register_rel_addr(reg), m_s->x[reg], val, "x");
    }

    uint64_t do_read_pc(void) {
        return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::pc), m_s->pc, "pc");
    }

    void do_write_pc(uint64_t val) {
        log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::pc), m_s->pc, val, "pc");
    }

	uint64_t do_read_minstret(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::minstret), m_s->minstret, "minstret");
	}

	void do_write_minstret(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::minstret), m_s->minstret, val, "minstret");
	}

	uint64_t do_read_mvendorid(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mvendorid), m_s->mvendorid, "mvendorid");
	}

	void do_write_mvendorid(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mvendorid), m_s->mvendorid, val, "mvendorid");
	}

	uint64_t do_read_marchid(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::marchid), m_s->marchid, "marchid");
	}

	void do_write_marchid(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::marchid), m_s->marchid, val, "marchid");
	}

	uint64_t do_read_mimpid(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mimpid), m_s->mimpid, "mimpid");
	}

	void do_write_mimpid(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mimpid), m_s->mimpid, val, "mimpid");
	}

	uint64_t do_read_mcycle(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mcycle), m_s->mcycle, "mcycle");
	}

	void do_write_mcycle(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mcycle), m_s->mcycle, val, "mcycle");
	}

	uint64_t do_read_mstatus(void) {
        return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mstatus), m_s->mstatus, "mstatus");
	}

	void do_write_mstatus(uint64_t val) {
        log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mstatus), m_s->mstatus, val, "mstatus");
	}

	uint64_t do_read_mtvec(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mtvec), m_s->mtvec, "mtvec");
	}

	void do_write_mtvec(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mtvec), m_s->mtvec, val, "mtvec");
	}

	uint64_t do_read_mscratch(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mscratch), m_s->mscratch, "mscratch");
	}

	void do_write_mscratch(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mscratch), m_s->mscratch, val, "mscratch");
	}

	uint64_t do_read_mepc(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mepc), m_s->mepc, "mepc");
	}

	void do_write_mepc(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mepc), m_s->mepc, val, "mepc");
	}

	uint64_t do_read_mcause(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mcause), m_s->mcause, "mcause");
	}

	void do_write_mcause(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mcause), m_s->mcause, val, "mcause");
	}

	uint64_t do_read_mtval(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mtval), m_s->mtval, "mtval");
	}

	void do_write_mtval(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mtval), m_s->mtval, val, "mtval");
	}

	uint64_t do_read_misa(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::misa), m_s->misa, "misa");
	}

	void do_write_misa(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::misa), m_s->misa, val, "misa");
	}

	uint64_t do_read_mie(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mie), m_s->mie, "mie");
	}

	void do_write_mie(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mie), m_s->mie, val, "mie");
	}

	uint64_t do_read_mip(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mip), m_s->mip, "mip");
	}

	void do_write_mip(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mip), m_s->mip, val, "mip");
	}

	uint64_t do_read_medeleg(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::medeleg), m_s->medeleg, "medeleg");
	}

	void do_write_medeleg(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::medeleg), m_s->medeleg, val, "medeleg");
	}

	uint64_t do_read_mideleg(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mideleg), m_s->mideleg, "mideleg");
	}

	void do_write_mideleg(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mideleg), m_s->mideleg, val, "mideleg");
	}

	uint64_t do_read_mcounteren(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mcounteren), m_s->mcounteren, "mcounteren");
	}

	void do_write_mcounteren(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::mcounteren), m_s->mcounteren, val, "mcounteren");
	}

	uint64_t do_read_stvec(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::stvec), m_s->stvec, "stvec");
	}

	void do_write_stvec(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::stvec), m_s->stvec, val, "stvec");
	}

	uint64_t do_read_sscratch(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::sscratch), m_s->sscratch, "sscratch");
	}

	void do_write_sscratch(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::sscratch), m_s->sscratch, val, "sscratch");
	}

	uint64_t do_read_sepc(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::sepc), m_s->sepc, "sepc");
	}

	void do_write_sepc(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::sepc), m_s->sepc, val, "sepc");
	}

	uint64_t do_read_scause(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::scause), m_s->scause, "scause");
	}

	void do_write_scause(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::scause), m_s->scause, val, "scause");
	}

	uint64_t do_read_stval(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::stval), m_s->stval, "stval");
	}

	void do_write_stval(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::stval), m_s->stval, val, "stval");
	}

	uint64_t do_read_satp(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::satp), m_s->satp, "satp");
	}

	void do_write_satp(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::satp), m_s->satp, val, "satp");
	}

	uint64_t do_read_scounteren(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::scounteren), m_s->scounteren, "scounteren");
	}

	void do_write_scounteren(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::scounteren), m_s->scounteren, val, "scounteren");
	}

	uint64_t do_read_ilrsc(void) {
		return log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::ilrsc), m_s->ilrsc, "ilrsc");
	}

	void do_write_ilrsc(uint64_t val) {
		log_before_write_write_and_update(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::ilrsc), m_s->ilrsc, val, "ilrsc");
	}
    ...
    void do_set_iflags_H(void) {
        ...
        uint64_t iflags_addr = m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::iflags);
    ...
    bool do_read_iflags_H(void) {
        log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::iflags), machine_read_iflags(m_s), "iflags_H");
        return m_s->iflags_H;
    }
    ...
    bool do_read_iflags_I(void) {
        log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::iflags), machine_read_iflags(m_s), "iflags_I");
        return m_s->iflags_I;
    }

    uint8_t do_read_iflags_PRV(void) {
        log_read(m_s->shadow_pma->start + shadow_get_csr_rel_addr(shadow_csr::iflags), machine_read_iflags(m_s), "iflags_PRV");
    ...
    uint64_t do_read_clint_mtimecmp(void) {
		return log_read(m_s->clint_pma->start + clint_get_csr_rel_addr(clint_csr::mtimecmp), m_s->clint_mtimecmp, "clint_mtimecmp");
    }

    void do_write_clint_mtimecmp(uint64_t val) {
        log_before_write_write_and_update(m_s->clint_pma->start + clint_get_csr_rel_addr(clint_csr::mtimecmp), m_s->clint_mtimecmp, val, "clint_mtimecmp");
    }

    uint64_t do_read_htif_fromhost(void) {
        return log_read(m_s->htif_pma->start + htif_get_csr_rel_addr(htif_csr::fromhost), m_s->htif_fromhost, "htif_fromhost");
    }

    void do_write_htif_fromhost(uint64_t val) {
        log_before_write_write_and_update(m_s->htif_pma->start + htif_get_csr_rel_addr(htif_csr::fromhost), m_s->htif_fromhost, val, "htif_fromhost");
    }

    uint64_t do_read_htif_tohost(void) {
        return log_read(m_s->htif_pma->start + htif_get_csr_rel_addr(htif_csr::tohost), m_s->htif_tohost, "htif_tohost");
    }
