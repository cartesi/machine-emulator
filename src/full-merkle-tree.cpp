...
full_merkle_tree::full_merkle_tree(int log2_root_size, int log2_leaf_size,
    int log2_word_size):
...
full_merkle_tree::full_merkle_tree(int log2_root_size, int log2_leaf_size,
    int log2_word_size, const std::vector<hash_type> &leaves):
    m_log2_root_size(log2_root_size),
    m_log2_leaf_size(log2_leaf_size),
    m_max_leaves{address_type{1} << std::max(0,
        log2_root_size-log2_leaf_size)} {
...
full_merkle_tree::proof_type
full_merkle_tree::get_proof(address_type address, int log2_size) const {
...
void
full_merkle_tree::check_log2_sizes(int log2_root_size, int log2_leaf_size,
    int log2_word_size) {
    if (log2_root_size < 0) {
...
void
full_merkle_tree::init_pristine_subtree(const pristine_merkle_tree &pristine,
...
void
full_merkle_tree::init_subtree(hasher_type &h, int index, int log2_size) {
    if (log2_size > get_log2_leaf_size()) {
...
void
full_merkle_tree::init_tree(const pristine_merkle_tree &pristine,
...
full_merkle_tree::address_type
full_merkle_tree::get_node_index(address_type address, int log2_size) const {
    if (log2_size < get_log2_leaf_size() ||
        log2_size > get_log2_root_size()) {
        throw std::out_of_range{"log2_size is out of bounds"};
    }
