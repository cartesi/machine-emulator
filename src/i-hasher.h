...
public:
    ...
    void add_data(D &&data) noexcept { // NOLINT(cppcoreguidelines-missing-std-forward)
        derived().do_add_data(
    ...
    template <ContiguousRangeOfByteLike D>
    static void hash(D &&data, machine_hash_view hash) noexcept { // NOLINT(cppcoreguidelines-missing-std-forward)
        return DERIVED::do_hash(
            std::span<const uint8_t>{std::bit_cast<const uint8_t *>(std::ranges::data(data)), std::ranges::size(data)},
            hash);
    }

    static void hash(const_hash_tree_word_view data, machine_hash_view hash) noexcept {
    ...
    static void concat_hash(const_machine_hash_view data1, const_machine_hash_view data2,
        machine_hash_view hash) noexcept {
