...
// The preserve_none calling convention removes the callee-saved register
// burden from handlers: no prologue register saves on the hot path, and its
// argument registers (x20-x28 on AArch64) are callee-saved in the standard
// convention, so handler state survives cold calls into ordinary functions
// without spilling.
#if defined(__clang__) && __has_attribute(preserve_none)
#define TC_CALLCONV __attribute__((preserve_none))
// The next-instruction pre-load pays for itself only when the calling
// convention keeps the extra predicted state (word, dispatch target) in
...
static FORCE_INLINE fetch_status tc_fetch_insn(const STATE_ACCESS a, uint64_t &pc, uint32_t &insn,
    ...
    if (pc_vaddr_page == fetch_vaddr_page) [[unlikely]] {
        ...
        i_state_access_fast_addr_t<STATE_ACCESS> pc_vf_offset{};
        uint64_t pc_pma_index{};
        if (tc_fetch_translate_pc(a, pc, pc, pc_vf_offset, pc_pma_index, tcc) == fetch_status::exception)
            [[unlikely]] {
...
interpreter_break_reason interpret(const STATE_ACCESS a, uint64_t mcycle_end) {
    ...
    const execute_status status = [&] {
        if constexpr (std::is_same_v<STATE_ACCESS, state_access>) {
            return interpret_loop_tc(a, effective_mcycle_end, mcycle);
        } else {
