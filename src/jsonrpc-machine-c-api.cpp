...
int cm_jsonrpc_verify_load_cmio_input_log(const cm_jsonrpc_mg_mgr *mgr, uint16_t reason, const unsigned char *data,
    size_t length, const cm_access_log *log, const cm_machine_runtime_config *runtime_config, bool one_based,
    char **err_msg) try {
    const auto *cpp_mgr = convert_from_c(mgr);
    const cartesi::access_log cpp_log = convert_from_c(log);
    const cartesi::machine_runtime_config cpp_runtime = convert_from_c(runtime_config);
    cartesi::jsonrpc_virtual_machine::verify_load_cmio_input_log(*cpp_mgr, reason, data, length, cpp_log, cpp_runtime,
        one_based);
    return cm_result_success(err_msg);
} catch (...) {
    return cm_result_failure(err_msg);
...
int cm_jsonrpc_verify_load_cmio_input_state_transition(const cm_jsonrpc_mg_mgr *mgr, uint16_t reason,
    ...
    const cartesi::machine::hash_type cpp_root_hash_after = convert_from_c(root_hash_after);
    const cartesi::machine_runtime_config cpp_runtime = convert_from_c(runtime_config);
    cartesi::jsonrpc_virtual_machine::verify_load_cmio_input_state_transition(*cpp_mgr, reason, data, length,
