...
/// \brief JSONRPC handler for the machine.load_cmio_input method
/// \param j JSON request object
/// \param con Mongoose connection
/// \param h Handler data
/// \returns JSON response object
static json jsonrpc_machine_load_cmio_input_handler(const json &j, mg_connection *con, http_handler_data *h) {
    (void) con;
    if (!h->machine) {
        return jsonrpc_response_invalid_request(j, "no machine");
    }
    ...
    auto bin = cartesi::decode_base64(std::get<1>(args));
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    h->machine->load_cmio_input(std::get<0>(args), reinterpret_cast<unsigned char *>(bin.data()), bin.size());
    return jsonrpc_response_ok(j);
}

static json jsonrpc_machine_log_load_cmio_input_handler(const json &j, mg_connection *con, http_handler_data *h) {
    (void) con;
    if (!h->machine) {
        return jsonrpc_response_invalid_request(j, "no machine");
    }
    ...
    switch (count_args(args)) {
        case 3:
            s = jsonrpc_response_ok(j,
                h->machine->log_load_cmio_input(std::get<0>(args), reinterpret_cast<unsigned char *>(bin.data()),
...
/// \brief JSONRPC handler for the machine.verify_load_cmio_input_log method
/// \param j JSON request object
/// \param con Mongoose connection
/// \param h Handler data
/// \returns JSON response object
static json jsonrpc_machine_verify_load_cmio_input_log_handler(const json &j, mg_connection *con,
    http_handler_data *h) {
    (void) con;
    (void) h;
    ...
    switch (count_args(args)) {
        case 3:
            cartesi::machine::verify_load_cmio_input_log(std::get<0>(args),
...
/// \brief JSONRPC handler for the machine.verify_load_cmio_input_state_transition method
/// \param j JSON request object
/// \param con Mongoose connection
/// \param h Handler data
/// \returns JSON response object
static json jsonrpc_machine_verify_load_cmio_input_state_transition_handler(const json &j, mg_connection *con,
    http_handler_data *h) {
    (void) con;
    (void) h;
    ...
    switch (count_args(args)) {
        case 5:
            cartesi::machine::verify_load_cmio_input_state_transition(std::get<0>(args),
...
static json jsonrpc_dispatch_method(const json &j, mg_connection *con, http_handler_data *h) try {
    static const std::unordered_map<std::string, jsonrpc_handler> dispatch = {
        ...
        {"machine.load_cmio_input", jsonrpc_machine_load_cmio_input_handler},
        {"machine.log_load_cmio_input", jsonrpc_machine_log_load_cmio_input_handler},
        {"machine.verify_load_cmio_input_log", jsonrpc_machine_verify_load_cmio_input_log_handler},
        {"machine.verify_load_cmio_input_state_transition",
            jsonrpc_machine_verify_load_cmio_input_state_transition_handler},
    };
