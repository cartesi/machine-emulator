...
void jsonrpc_virtual_machine::do_load_cmio_input(uint16_t reason, const unsigned char *data, size_t length) {
    bool result = false;
    std::string b64 = cartesi::encode_base64(data, length);
    jsonrpc_request(m_mgr->get_mgr(), m_mgr->get_remote_address(), "machine.load_cmio_input", std::tie(reason, b64),
        result);
}

access_log jsonrpc_virtual_machine::do_log_load_cmio_input(uint16_t reason, const unsigned char *data, size_t length,
    const access_log::type &log_type, bool one_based) {
    not_default_constructible<access_log> result;
    std::string b64 = cartesi::encode_base64(data, length);
    jsonrpc_request(m_mgr->get_mgr(), m_mgr->get_remote_address(), "machine.log_load_cmio_input",
...
void jsonrpc_virtual_machine::verify_load_cmio_input_log(const jsonrpc_mg_mgr_ptr &mgr, uint16_t reason,
    ...
    jsonrpc_request(mgr->get_mgr(), mgr->get_remote_address(), "machine.verify_load_cmio_input_log",
        std::tie(reason, b64_data, log, runtime, one_based), result);
}

void jsonrpc_virtual_machine::verify_load_cmio_input_state_transition(const jsonrpc_mg_mgr_ptr &mgr, uint16_t reason,
    ...
    auto b64_root_hash_before = encode_base64(root_hash_before);
    auto b64_root_hash_after = encode_base64(root_hash_after);
    jsonrpc_request(mgr->get_mgr(), mgr->get_remote_address(), "machine.verify_load_cmio_input_state_transition",
