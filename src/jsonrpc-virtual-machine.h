...
public:
    ...
    static void verify_load_cmio_input_log(const jsonrpc_mg_mgr_ptr &mgr, uint16_t reason, const unsigned char *data,
        size_t length, const access_log &log, const machine_runtime_config &r = {}, bool one_based = false);

    static void verify_load_cmio_input_state_transition(const jsonrpc_mg_mgr_ptr &mgr, uint16_t reason,
...
private:
    ...
    void do_load_cmio_input(uint16_t reason, const unsigned char *data, size_t length) override;
    access_log do_log_load_cmio_input(uint16_t reason, const unsigned char *data, size_t length,
