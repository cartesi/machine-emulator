...
void machine::init_tlb() {
    for (auto use : {TLB_CODE, TLB_READ, TLB_WRITE}) {
        ...
        for (uint64_t slot_index = 0; slot_index < TLB_SET_SIZE; ++slot_index) {
            ...
            hot_slot.vh_offset = machine_haddr{};
    ...
    for (auto use : {TLB_CODE, TLB_READ, TLB_WRITE}) {
        ...
        for (uint64_t slot_index = 0; slot_index < TLB_SET_SIZE; ++slot_index) {
            ...
            if (shadow_slot.vaddr_page != TLB_INVALID_PAGE) {
                ...
                hot_slot.vh_offset = get_haddr(paddr_page, pma_index) - vaddr_page;
            } else {
...
uint64_t machine::get_paddr(machine_haddr haddr, uint64_t pma_index) const {
    return static_cast<uint64_t>(haddr + get_hp_offset(pma_index));
}

machine_haddr machine::get_haddr(uint64_t paddr, uint64_t pma_index) const {
    return machine_haddr{paddr} - get_hp_offset(pma_index);
}

void machine::mark_dirty_page(machine_haddr haddr, uint64_t pma_index) {
    auto paddr = get_paddr(haddr, pma_index);
    auto &pma = m_s.pmas[static_cast<int>(pma_index)];
    pma.mark_dirty_page(paddr - pma.get_start());
}

machine_haddr machine::get_hp_offset(uint64_t pma_index) const {
    if (pma_index >= m_s.pmas.size()) {
    ...
    auto haddr = cast_ptr_to_haddr(pma.get_memory().get_host_memory());
