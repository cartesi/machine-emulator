...
void machine::init_pmas_contents(const pmas_config &config) {
    ...
    if (config.backing_store.data_filename.empty() || config.backing_store.create) {
        ...
        std::ranges::transform(m_ars.pmas_view(), dest.begin(),
...
static void store_hash(const machine_hash_view &h, const std::string &dir) {
    auto name = dir + "/hash";
...
std::optional<std::span<const unsigned char>> ar_host_memory_view(const address_range &ar,
    ...
    if (ar.is_memory()) {
        if (ar.get_host_memory() == nullptr) {
            throw std::runtime_error{"memory address range "s.append(ar.get_description()).append("has no host memory")};
        }
