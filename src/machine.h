...
private:
    ...
    machine_haddr get_hp_offset(uint64_t pma_index) const;
...
public:
    ...
    uint64_t get_paddr(machine_haddr haddr, uint64_t pma_index) const;
    ...
    machine_haddr get_haddr(uint64_t paddr, uint64_t pma_index) const;

    /// \brief Marks a page as dirty
    /// \param haddr Machine host address within page
    /// \param pma_index Index of PMA where address falls
    void mark_dirty_page(machine_haddr haddr, uint64_t pma_index);
