...
void change_writable(const std::string &pathname, bool writable) {
    // Open the path
    const auto fd = open(pathname.c_str(), O_RDONLY | O_BINARY);
    if (fd < 0) {
...
void truncate_file(const std::string &filename, uint64_t size, bool create) {
    ...
    const int flags = O_WRONLY | O_BINARY | (create ? (O_CREAT | O_EXCL) : 0);
...
std::pair<std::unique_ptr<uint8_t[]>, std::span<uint8_t>> read_file(const std::string &filename) {
    // Open file
    const auto fd = open(filename.c_str(), O_RDONLY | O_BINARY);
    if (fd < 0) {
...
void create_file(const std::string &filename, std::span<const uint8_t> data) {
    // Create file
    const auto fd = open(filename.c_str(), O_WRONLY | O_BINARY | O_CREAT | O_EXCL, S_CREATE_FILE);
    if (fd < 0) {
    ...
    while (offset < data.size()) {
        ...
        if (!is_pristine(data.data() + offset, bytes_to_write)) {
...
void copy_file(const std::string &from, const std::string &to, uint64_t size) {
    // Open source file
    const auto from_fd = open(from.c_str(), O_RDONLY | O_BINARY);
    if (from_fd < 0) {
    ...
    const auto to_fd = open(to.c_str(), O_WRONLY | O_BINARY | O_CREAT | O_EXCL, st.st_mode & S_ACCESS_ALL);
    if (to_fd < 0) {
        throw make_copy_path_system_error(errno, "unable to copy file"s, from, to, "open() failed"s);
    }
    ...
    while (offset < copy_size) {
        ...
        if (!is_pristine(buffer.data(), bytes_read)) {
...
void reflink_file(const std::string &from, const std::string &to) {
    ...
    // Create destination file
    const auto to_fd = open(to.c_str(), O_WRONLY | O_BINARY | O_CREAT | O_EXCL, st.st_mode & ACCESSPERMS);
    if (to_fd < 0) {
        throw make_copy_path_system_error(errno, "unable to create reference link"s, from, to, "open() failed"s);
    }
