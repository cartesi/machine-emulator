...
static execute_status plic_write(void *context, i_device_state_access *a, uint64_t offset, uint64_t val,
    ...
    // PLIC doesn't support 64-bit writes, but there is a bug in our current BBL performing 64-bit writes.
    // This was fixed in BBL after https://github.com/riscv-software-src/riscv-pk/pull/205
    // As a workaround, lets just pretend it was a 32-bit write
    if (log2_size == 3) {
        log2_size = 2;
