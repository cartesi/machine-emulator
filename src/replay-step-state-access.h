...
// \brief checks if the buffer size is large enough to hold the data
...
// \param next Receives the offset of the next data
...
#pragma pack(pop)
    uint64_t m_page_count;                                     ///< Number of pages in the step log
    page_type *m_pages;                                        ///< Array of page data
    uint64_t m_sibling_count;                                  ///< Number of sibling hashes in the step log
    hash_type *m_sibling_hashes;                               ///< Array of sibling hashes
...
private:
    ...
    page_type *try_find_page(uint64_t address) const {
        ...
        while (min < max) {
            ...
            if (m_pages[mid].index == page_index) {
                return &m_pages[mid];
            } else if (m_pages[mid].index < page_index) {
                min = mid + 1;
            } else {
                max = mid;
            }
    ...
    page_type *find_page(uint64_t address) {
        ...
        return const_cast<page_type *>(static_cast<const replay_step_state_access *>(this)->find_page(address));
    }
    ...
    hash_type compute_root_hash_impl(address_type page_index, int page_count_log2_size, size_t &next_page,
        ...
        if (next_page >= m_page_count || page_index + page_count <= m_pages[next_page].index) {
            ...
            hash_type hash{};
            interop_concat_hash(reinterpret_cast<const interop_hash_type>(&left),
                reinterpret_cast<const interop_hash_type>(&right), reinterpret_cast<interop_hash_type>(&hash));
            return hash;
        } else if (m_pages[next_page].index == page_index) {
