...
private:
    ...
    /// \return A pointer to the page_type structure if found, nullptr otherwise
    page_type *try_find_page(machine_haddr haddr_page) const {
        uint64_t min{0};
        uint64_t max{m_page_count};
        while (min < max) {
            auto mid = (min + max) >> 1;
            auto mid_page_data = cast_ptr_to_haddr(m_pages[mid].data);
    ...
    void relocate_tlb_vp_offset_to_vh_offset() {
        for (uint64_t slot_index = 0; slot_index < TLB_SET_SIZE; ++slot_index) {
            ...
            const auto vaddr_page_field_haddr =
                cast_ptr_to_haddr(vaddr_page_log->data) + (vaddr_page_field_addr & PAGE_OFFSET_MASK);
            ...
            if (vaddr_page != TLB_INVALID_PAGE) {
                const auto vp_offset_field_haddr =
                    cast_ptr_to_haddr(vp_offset_log->data) + (vp_offset_field_addr & PAGE_OFFSET_MASK);
                ...
                const auto haddr_page = cast_ptr_to_haddr(page_log->data);
                const auto vh_offset = haddr_page - vaddr_page;
                aliased_aligned_write<machine_haddr>(vp_offset_field_haddr, vh_offset);
            }
        }
    }
    ...
    void relocate_tlb_vh_offset_to_vp_offset() {
        for (uint64_t slot_index = 0; slot_index < TLB_SET_SIZE; ++slot_index) {
            ...
            if (vaddr_page != TLB_INVALID_PAGE) {
                const auto vp_offset_field_haddr =
                    cast_ptr_to_haddr(vp_offset_log->data) + (vp_offset_field_addr & PAGE_OFFSET_MASK);
                const auto vh_offset = aliased_aligned_read<machine_haddr>(vp_offset_field_haddr);
    ...
    page_type *find_page(machine_haddr haddr_page) const {
        auto *page_log = try_find_page(haddr_page);
        if (page_log == nullptr) {
            interop_throw_runtime_error("find_page: page not found");
        }
    ...
    /// \return Host address
    machine_haddr do_get_faddr(uint64_t paddr, uint64_t /* pma_index */ = 0) const {
        ...
        return cast_ptr_to_haddr(page_log->data) + offset;
    }
    ...
    machine_haddr do_read_tlb_vp_offset(uint64_t slot_index) {
        ...
        return aliased_aligned_read<machine_haddr>(haddr);
    }
    ...
    void do_write_tlb(uint64_t slot_index, uint64_t vaddr_page, machine_haddr vh_offset, uint64_t pma_index) {
        ...
        aliased_aligned_write<machine_haddr>(haddr_vp_offset, vh_offset);
