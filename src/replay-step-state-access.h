...
public:
    ...
    replay_step_state_access(context &context, unsigned char *log_image, uint64_t log_size,
        ...
        // hash tree target
        uint64_t temp_hash_tree_target{};
        if (!validate_and_advance_offset(log_size, 0, sizeof(temp_hash_tree_target), 1, &page_count_offset)) {
            interop_throw_runtime_error("hash tree target past end of step log");
        }
        memcpy(&temp_hash_tree_target, log_image, sizeof(temp_hash_tree_target));
        auto maybe_hash_tree_target = parse_hash_tree_target(temp_hash_tree_target);
        if (!maybe_hash_tree_target) {
            interop_throw_runtime_error("invalid log format: hash tree target not supported");
        }
        m_context.target = *maybe_hash_tree_target;
        ...
        memcpy(&m_context.page_count, log_image + page_count_offset, sizeof(m_context.page_count));
        ...
        // set sibling hashes
        if (!validate_and_advance_offset(log_size, first_siblng_offset, sizeof(machine_hash), m_context.sibling_count,
                &end_offset)) {
            interop_throw_runtime_error("sibling hashes past end of step log");
        }
        // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
        m_context.sibling_hashes = reinterpret_cast<machine_hash *>(log_image + first_siblng_offset);
...
private:
    ...
    machine_hash compute_root_hash_impl(address_type page_index, int page_count_log2_size, size_t &next_page,
        size_t &next_sibling) {
        // NOLINTBEGIN(cppcoreguidelines-pro-type-reinterpret-cast))
