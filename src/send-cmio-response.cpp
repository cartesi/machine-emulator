...
void send_cmio_response(STATE_ACCESS a, uint16 reason, bytes data, uint64 dataLength, bytes32 revertRootHash) {
    ...
    if (dataLength > 0) {
        ...
        if (dataLength > uint64ShiftLeft(1, uint32(AR_CMIO_RX_BUFFER_LOG2_SIZE))) {
            return;
        }
        // Find the write length: the smallest power of 2 that is >= dataLength and >= tree leaf size
        writeLengthLog2Size = uint32Log2(uint32(dataLength));
    ...
    uint64 yieldData = uint64ShiftLeft((uint64(reason) & mask16), 32) | (dataLength & mask32);
...
template void send_cmio_response(state_access a, uint16_t reason, const unsigned char *data, uint64 length,
    bytes32 revertRootHash);

// Explicit instantiation for record_step_state_access
template void send_cmio_response(record_step_state_access a, uint16_t reason, const unsigned char *data, uint64 length,
    bytes32 revertRootHash);
