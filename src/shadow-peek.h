...
static bool shadow_peek(READ_REG_F read_reg, const address_range &ar, const machine &m, uint64_t offset,
    uint64_t length, const unsigned char **data, unsigned char *scratch) {
    ...
    if (offset > offset_aligned) {
        const auto val = read_reg(m, ar.get_start() + offset_aligned);
    ...
    const uint64_t paddr_start = ar.get_start() + offset;
