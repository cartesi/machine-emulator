...
private:
    ...
    void do_write_mstatus(uint64_t val) const {
        ...
        m_s.penumbra.tlb_ctx_slot_base = tlb_ctx(m_s.shadow.registers.iprv, val & MSTATUS_SUM_MASK) * TLB_SET_SIZE;
    }
    ...
    void do_write_iprv(uint64_t val) const {
        ...
        m_s.penumbra.tlb_ctx_slot_base = tlb_ctx(val, m_s.shadow.registers.mstatus & MSTATUS_SUM_MASK) * TLB_SET_SIZE;
    }
