...
private:
    ...
    static void get_word_hash(merkle_tree::hasher_type &hasher,
        const access_data &word, merkle_tree::hash_type &word_hash) {
        assert(word.size() == 8);
        hasher.begin();
        hasher.add_data(word.data(), word.size());
    ...
    uint64_t check_read(uint64_t paligned, const char *text) {
        ...
        m_next_access++;
        return get_word_access_data(access.read);
    }
    ...
    void check_write(uint64_t paligned, uint64_t val, const char *text) {
        ...
        if (m_verify_proofs) {
            ...
            if (get_word_access_data(access.written) != val) {
