...
private:
    ...
    uint64_t do_read_x(int reg) {
        return check_read_word(PMA_SHADOW_START + shadow_get_x_rel_addr(reg), "x");
    }

    void do_write_x(int reg, uint64_t val) {
        assert(reg != 0);
        check_write_word(PMA_SHADOW_START + shadow_get_x_rel_addr(reg), val, "x");
    }

    uint64_t do_read_pc(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::pc), "pc");
    }

    void do_write_pc(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::pc), val, "pc");
    }

    uint64_t do_read_minstret(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::minstret), "minstret");
    }

    void do_write_minstret(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::minstret), val, "minstret");
    }

    uint64_t do_read_mvendorid(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mvendorid), "mvendorid");
    }

    uint64_t do_read_marchid(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::marchid), "marchid");
    }

    uint64_t do_read_mimpid(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mimpid), "mimpid");
    }

    uint64_t do_read_mcycle(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mcycle), "mcycle");
    }

    void do_write_mcycle(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mcycle), val, "mcycle");
    }

    uint64_t do_read_mstatus(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mstatus), "mstatus");
    }

    void do_write_mstatus(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mstatus), val, "mstatus");
    }

    uint64_t do_read_mtvec(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mtvec), "mtvec");
    }

    void do_write_mtvec(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mtvec), val, "mtvec");
    }

    uint64_t do_read_mscratch(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mscratch), "mscratch");
    }

    void do_write_mscratch(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mscratch), val, "mscratch");
    }

    uint64_t do_read_mepc(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mepc), "mepc");
    }

    void do_write_mepc(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mepc), val, "mepc");
    }

    uint64_t do_read_mcause(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mcause), "mcause");
    }

    void do_write_mcause(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mcause), val, "mcause");
    }

    uint64_t do_read_mtval(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mtval), "mtval");
    }

    void do_write_mtval(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mtval), val, "mtval");
    }

    uint64_t do_read_misa(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::misa), "misa");
    }

    void do_write_misa(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::misa), val, "misa");
    }

    uint64_t do_read_mie(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mie), "mie");
    }

    void do_write_mie(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mie), val, "mie");
    }

    uint64_t do_read_mip(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mip), "mip");
    }

    void do_write_mip(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mip), val, "mip");
    }

    uint64_t do_read_medeleg(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::medeleg), "medeleg");
    }

    void do_write_medeleg(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::medeleg), val, "medeleg");
    }

    uint64_t do_read_mideleg(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mideleg), "mideleg");
    }

    void do_write_mideleg(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mideleg), val, "mideleg");
    }

    uint64_t do_read_mcounteren(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::mcounteren), "mcounteren");
    }
    ...
    void do_write_stvec(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::stvec), val, "stvec");
    }

    uint64_t do_read_sscratch(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::sscratch), "sscratch");
    }

    void do_write_sscratch(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::sscratch), val, "sscratch");
    }

    uint64_t do_read_sepc(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::sepc), "sepc");
    }

    void do_write_sepc(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::sepc), val, "sepc");
    }

    uint64_t do_read_scause(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::scause), "scause");
    }

    void do_write_scause(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::scause), val, "scause");
    }

    uint64_t do_read_stval(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::stval), "stval");
    }

    void do_write_stval(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::stval), val, "stval");
    }

    uint64_t do_read_satp(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::satp), "satp");
    }

    void do_write_satp(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::satp), val, "satp");
    }

    uint64_t do_read_scounteren(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::scounteren), "scounteren");
    }
    ...
    uint64_t do_read_ilrsc(void) {
        return check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::ilrsc), "ilrsc");
    }

    void do_write_ilrsc(uint64_t val) {
        check_write_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::ilrsc), val, "ilrsc");
    }
    ...
    bool do_read_iflags_Y(void) {
        auto iflags = check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::iflags), "iflags.Y");
        return (iflags & IFLAGS_Y_MASK) != 0;
    }

    uint8_t do_read_iflags_PRV(void) {
        auto iflags = check_read_word(PMA_SHADOW_START + shadow_get_csr_rel_addr(shadow_csr::iflags), "iflags.PRV");
    ...
    uint64_t do_read_pma_ilength(int i) {
        auto rel_addr = shadow_get_pma_rel_addr(i);
        return check_read_word(PMA_SHADOW_START + rel_addr + sizeof(uint64_t), "pma.ilength");
    }
    ...
    pma_entry &build_mock_device_pma_entry(uint64_t start, uint64_t length, const pma_entry::flags &f) {
        switch (f.DID) {
            case PMA_ISTART_DID::shadow:
                return allocate_mock_pma_entry(make_shadow_pma_entry(start, length).set_flags(f));
