...
static inline void aliased_aligned_write(machine_haddr haddr, T v) {
    memcpy(__builtin_assume_aligned(cast_haddr_to_ptr(haddr), sizeof(A)), &v, sizeof(T));
}
...
/// \returns Value read.
template <typename T, typename A = T>
static inline T aliased_aligned_read(machine_haddr haddr) {
    T v;
    memcpy(&v, __builtin_assume_aligned(cast_haddr_to_ptr(haddr), sizeof(A)), sizeof(T));
    return v;
}
