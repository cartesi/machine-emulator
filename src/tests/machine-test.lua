...
local function build_machine_with_flash()
    ...
    flash_drive_config = {
        ...
        length = 0x3c00000,
        shared = false,
        image_filename = test_utils.images_path .. "rootfs.ext2"
    }
...
local do_test_with_flash = test_utils.make_do_test(build_machine_with_flash)
...
local function calculate_emulator_hash(pmas_files)
    ...
    local cpu_and_rom_data_pages = (procesor_board_shadow.data_size +
                                       rom.data_size) / (2 ^ 12)
    local cpu_and_rom_space_hash = test_utils.calculate_region_hash(
                                       cpu_and_rom_data, cpu_and_rom_data_pages,
                                       12, 16)
    cpu_and_rom_space_hash = test_utils.extend_region_hash(
                                 cpu_and_rom_space_hash, 0x0, 16, 25)

    local cli_space_hash = test_utils.calculate_region_hash(cli.data,
                                                            cli.data_size /
                                                                (2 ^ 12), 12, 14)
    cli_space_hash = test_utils.extend_region_hash(cli_space_hash, 0x02000000,
                                                   14, 25)

    local cpu_rom_cli_hash = cartesi.keccak(cpu_and_rom_space_hash,
                                            cli_space_hash) -- 26
    cpu_rom_cli_hash = test_utils.extend_region_hash(cpu_rom_cli_hash, 0x0, 26,
                                                     30)

    local hti_log2_data_size = math.log(hti.data_size, 2)
    local hti_space_hash = test_utils.calculate_region_hash_2(0x40008000,
                                                              hti.data,
                                                              hti_log2_data_size,
                                                              30)

    local ram_log2_data_size = math.log(ram.data_size, 2)
    local ram_space_hash = test_utils.calculate_region_hash_2(0x80000000,
                                                              ram.data,
                                                              ram_log2_data_size,
                                                              31)
    ...
    local total_space_hash = test_utils.extend_region_hash(used_space_hash, 0x0,
                                                           32, 64)
...
do_test("machine halt and yield flags and config matches", function(machine)
    ...
    -- test_utils.print_table(initial_config)
...
do_test("dumped file merkle tree hashes should match", function(machine)
    ...
    for _, file_name in pairs(pmas_file_names) do
        ...
        local temp = test_utils.split_string(file_name, "--.")
        ...
        root_file_hashes[file_name] = test_utils.calculate_region_hash(pmas_file.data,
                                                                       data_number_of_pages,
                                                                       page_log2_size,
                                                                       tree_log2_size)
        print("\nHash of address space: ", file_name, " of log size:",
              tree_log2_size, " is ",
              test_utils.tohex(root_file_hashes[file_name]))
    end
    assert(test_utils.tohex(root_file_hashes[pmas_file_names[1]]) ==
               "E0387504AF856C8FDD164CA2EB44FEA5BC4094232D86C86021450687C9180863")
    assert(test_utils.tohex(root_file_hashes[pmas_file_names[2]]) ==
               "3DF875E72A8B29E6F0DB9CFE2934F7B80E7DB855C1729CF140D358A72B9D73BD")
    assert(test_utils.tohex(root_file_hashes[pmas_file_names[3]]) ==
               "995C871A78EFEC6CA5AFD44B9994B1C88BBBFCDFEA68FD5566C13D4F45BBDE6B")
    assert(test_utils.tohex(root_file_hashes[pmas_file_names[4]]) ==
               "3279ED2C35ADE5BCFFC6680AA2E08153D2EDD5A9949ECA9E731B3F5DCE2721A0")
    assert(test_utils.tohex(root_file_hashes[pmas_file_names[5]]) ==
...
do_test("machine initial hash shold match", function(machine)
    ...
    print("Root hash:", test_utils.tohex(root_hash), " calculated root hash:",
          test_utils.tohex(calculated_root_hash))

    assert(
        test_utils.tohex(root_hash) == test_utils.tohex(calculated_root_hash),
...
do_test("machine root hash after step one shold match", function(machine)
    ...
    assert(test_utils.tohex(root_hash_step1) ==
               test_utils.tohex(calculated_root_hash_step1),
...
do_test("proof check should pass", function(machine)
    ...
    local ram_address_start = tonumber(test_utils.split_string(ram_file_name,
                                                               "--.")[1], 16)
    local ram_data_number_of_pages = ram.data_size / (2 ^ 12)
    local ram_log2_data_size = math.log(ram.data_size, 2)
    local calculated_ram_hash = test_utils.calculate_region_hash(ram.data,
    ...
    assert(test_utils.tohex(root_hash) == test_utils.tohex(ram_proof.root_hash),
           "root hash in proof does not match")
    print("target hash:", test_utils.tohex(ram_proof.target_hash),
          " calculated target hash:", test_utils.tohex(calculated_ram_hash))
    assert(test_utils.tohex(calculated_ram_hash) ==
               test_utils.tohex(ram_proof.target_hash),
...
do_test("mcycle and root hash should match", function(machine)
    ...
    machine:update_merkle_tree()
    local root_hash = machine:get_root_hash()
    print("1000 cycle hash: ", test_utils.tohex(root_hash))
    assert(test_utils.tohex(root_hash) ==
...
do_test("mcycle and root hash should match", function(machine)
    ...
    machine:update_merkle_tree()
    local root_hash = machine:get_root_hash()
    print("End hash: ", test_utils.tohex(root_hash))
    assert(test_utils.tohex(root_hash) ==
...
do_test("proof  and root hash should match", function(machine)
    ...
    local initial_calculated_hash = test_utils.calculate_root_hash(
                                        initial_memory_read, 10)
    assert(test_utils.tohex(initial_ram_proof.target_hash) ==
               test_utils.tohex(initial_calculated_hash),
           "initial hash does not match")

    print("initial target hash:",
          test_utils.tohex(initial_ram_proof.target_hash),
    ...
    local calculated_hash = test_utils.calculate_root_hash(memory_read, 10)

    print("end target hash:", test_utils.tohex(ram_proof.target_hash),
          " calculated end target hash:", test_utils.tohex(calculated_hash))

    assert(test_utils.tohex(initial_ram_proof.target_hash) ~=
               test_utils.tohex(ram_proof.target_hash),
           "hash is same after memory is written")

    assert(test_utils.tohex(initial_calculated_hash) ~=
               test_utils.tohex(calculated_hash),
           "calculated hash is same after memory is written")

    assert(test_utils.tohex(ram_proof.target_hash) ==
               test_utils.tohex(calculated_hash),
