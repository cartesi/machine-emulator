...
public:
    ...
    explicit uarch_replay_step_state_access(const access_log &log, bool verify_proofs, bool one_based) :
...
private:
    ...
    const access_data &check_read(uint64_t paligned, int log2_size, const char *text) {
        ...
        machine_merkle_tree::hash_type computed_hash{};
        ...
        if (m_verify_proofs) {
            ...
            get_hash(m_hasher, access.get_read().value(), rolling_hash);
    ...
    void check_write(uint64_t paligned, const access_data &val, int log2_size, const char *text) {
        ...
        if (!access.get_read().has_value()) {
            throw std::invalid_argument{
                "missing read data from " + std::string(text) + " at access " + std::to_string(access_to_report())};
        }
        const auto &value_read = access.get_read().value(); // NOLINT(bugprone-unchecked-optional-access)
        if (value_read.size() != UINT64_C(1) << log2_size) {
        ...
        if (!access.get_written().has_value()) {
            throw std::invalid_argument{
                "missing written data from " + std::string(text) + " at access " + std::to_string(access_to_report())};
        }
        ...
        // check if logged written data hashes to the logged written hash
        ...
        if (m_verify_proofs) {
            ...
            auto value_written = access.get_written().value();
            get_hash(m_hasher, value_written, m_root_hash);
