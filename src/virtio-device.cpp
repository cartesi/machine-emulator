...
void virtio_device::poll_before_select(int *pmaxfd, fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
    timeval *timeout) {
    (void) pmaxfd;
    (void) readfds;
    (void) writefds;
...
/// \brief Called when the interpreter finished waiting file descriptors.
/// \returns True if an interrupt was requested, false otherwise.
/// \details This function should process pending events and trigger interrupt requests (if any).
bool virtio_device::poll_after_select(i_device_state_access *a, fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
    int select_ret) {
    (void) a;
    (void) readfds;
    (void) writefds;
...
bool virtio_device::poll_nowait(i_device_state_access *a) {
    int maxfd = -1;
    fd_set readfds{};
    fd_set writefds{};
    fd_set exceptfds{};
    ...
    FD_ZERO(&writefds);
    FD_ZERO(&exceptfds);
    poll_before_select(&maxfd, &readfds, &writefds, &exceptfds, &timeout);
    const int select_ret = select(maxfd + 1, &readfds, &writefds, &exceptfds, &timeout);
    return poll_after_select(a, &readfds, &writefds, &exceptfds, select_ret);
}
