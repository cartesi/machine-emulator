...
public:
    ...
    virtual void poll_before_select(int *pmaxfd, fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
        timeval *timeout);
    ...
    virtual bool poll_after_select(i_device_state_access *a, fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
        int select_ret);

    /// \brief Poll pending events without waiting (non-blocking).
    /// \details Basically call poll_before_select(), select() and poll_after_select() with timeout set to 0.
    /// \returns True if an interrupt was requested, false otherwise.
    bool poll_nowait(i_device_state_access *a);
