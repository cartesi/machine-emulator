...
static int close_fid_state(p9_fid_state *fidp) {
    if (!fidp || fidp->fid_type == P9_FID_NONE) {
        return 0;
    }
    int err = 0;
...
virtio_p9fs_device::virtio_p9fs_device(uint32_t virtio_idx, const std::string &mount_tag,
    ...
    if (root_path.length() + 1 >= P9_ROOT_PATH_MAX) {
        throw std::runtime_error{"shared path length is too large"};
    }
    if (mount_tag.length() >= P9_MOUNT_TAG_MAX) {
        throw std::runtime_error{"shared path mount tag length is too large"};
    }
    stat_t st{};
    if (stat(root_path.c_str(), &st) < 0 || !S_ISDIR(st.st_mode)) {
        throw std::runtime_error{"shared path '" + root_path + "' is not a valid directory"};
    }
...
bool virtio_p9fs_device::op_lopen(virtq_unserializer &&msg, uint16_t tag) {
    ...
    // Open the file
    int fd = -1;
    p9_fid_type fid_type = P9_FID_NONE;
    DIR *dirp = nullptr;
    if (flags & P9_O_DIRECTORY) {
        ...
        fd = open(fidp->path.c_str(), p9_open_flags_to_host(flags));
        if (fd < 0) {
            return send_error(msg, tag, host_errno_to_p9(errno));
        }
...
bool virtio_p9fs_device::op_lcreate(virtq_unserializer &&msg, uint16_t tag) {
    ...
    const int fd = open(path.c_str(), p9_open_flags_to_host(flags) | O_CREAT, static_cast<mode_t>(mode));
    if (fd < 0) {
        return send_error(msg, tag, host_errno_to_p9(errno));
    }
...
bool virtio_p9fs_device::op_readdir(virtq_unserializer &&msg, uint16_t tag) {
    ...
    // Get the fid state
    p9_fid_state *fidp = get_fid_state(fid);
    if (!fidp || fidp->fid_type != P9_FID_DIR) {
        return send_error(msg, tag, P9_EPROTO);
    }
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    DIR *dirp = reinterpret_cast<DIR *>(fidp->dirp);
...
bool virtio_p9fs_device::op_attach(virtq_unserializer &&msg, uint16_t tag) {
    ...
    *newfidp = p9_fid_state{uid, m_root_path, P9_FID_NONE};
    return true;
}
...
bool virtio_p9fs_device::op_walk(virtq_unserializer &&msg, uint16_t tag) {
    ...
    *newfidp = p9_fid_state{uid, path, P9_FID_NONE};
    return true;
}
...
bool virtio_p9fs_device::op_read(virtq_unserializer &&msg, uint16_t tag) {
    ...
    const p9_fid_state *fidp = get_fid_state(fid);
    if (!fidp || fidp->fid_type != P9_FID_FILE) {
        return send_error(msg, tag, P9_EPROTO);
    }
