...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // We'll build these after creating the machine, but consume the bytes now
    ...
    if (code_size < 4) {
        return 0; // need at least one instruction
    }
    ...
    if (enable_vm && pt_data_size > 0) {
        // Write raw fuzz bytes as page table data.
        // To give the fuzzer a fighting chance of producing valid translations,
        // we also set up a minimal identity-ish mapping for the code region.
        cm_write_memory(machine, RAM_START, pt_data, pt_data_size);

        // Build a minimal valid SV39 L2->L1->L0 mapping so code at CODE_START
        // is reachable. The fuzzer can still corrupt these via the pt_data above.
        ...
        // Virtual address we want to map: CODE_START (identity map)
        // SV39 splits: vpn[2] = bits[38:30], vpn[1] = bits[29:21], vpn[0] = bits[20:12]
        ...
        // L0 entry -> leaf mapping code page with RWX+U+A+D
        ...
        // Set satp to SV39 mode pointing at root page table
    ...
    // Build mstatus: start from fuzzed value, then apply control flags
    uint64_t mstatus = csrs.mstatus;
    // Set privilege in MPP field for MPRV effective privilege
    mstatus = (mstatus & ~(UINT64_C(3) << 11)) | (priv << 11);
    if (enable_mprv) {
        mstatus |= (UINT64_C(1) << 17); // MPRV
    } else {
        mstatus &= ~(UINT64_C(1) << 17);
    }
    if (enable_sum) {
        mstatus |= (UINT64_C(1) << 18); // SUM
    } else {
        mstatus &= ~(UINT64_C(1) << 18);
    }
    if (enable_mxr) {
        mstatus |= (UINT64_C(1) << 19); // MXR
    } else {
        mstatus &= ~(UINT64_C(1) << 19);
    }
    if (enable_fs) {
        mstatus |= (UINT64_C(3) << 13); // FS=Dirty (enable FP instructions)
    }
    ...
    // Set SENVCFG, MCOUNTEREN, SCOUNTEREN to fuzzed values from unused CSR bits
    cm_write_reg(machine, CM_REG_SENVCFG, csrs.menvcfg ^ csrs.mstatus); // derived from fuzz
    ...
    // Set FCSR from fuzz data (lower 8 bits are meaningful: frm + fflags)
