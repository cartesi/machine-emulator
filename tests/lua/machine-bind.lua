...
do_test("log_step sanity check", function(machine)
    ...
    module.machine.verify_step(root_hash_before, filename1, mcycle_count, root_hash_after)
    -- with incorrect hash args, verify step should fail
    local bad_hash = string.rep("\0", 32)
    _, err = pcall(function()
        module.machine.verify_step(bad_hash, filename1, mcycle_count, root_hash_after)
    end)
    assert(err:match("initial root hash mismatch"))
    _, err = pcall(function()
        module.machine.verify_step(root_hash_before, filename1, mcycle_count, bad_hash)
    end)
