...
do_test("machine run at the current cycle should not overflow", function(machine)
...
do_test("mcycle past the limit should overflow without advancing", function(machine)
    ...
    local hash_before = machine:get_root_hash()
    assert(machine:run(mcycle) == cartesi.BREAK_REASON_REACHED_TARGET_MCYCLE)
    assert(machine:read_reg("iflags_H") == 0)
    assert(machine:get_root_hash() == hash_before)
...
do_test("logged mcycle steps should preserve target and overflow semantics", function(machine)
    ...
    assert(machine:log_step(0, zero_step_filename) == cartesi.BREAK_REASON_REACHED_TARGET_MCYCLE)
...
do_test("logged mcycle step should do nothing at max mcycle", function(machine)
    ...
    assert(machine:log_step(1, step_filename) == cartesi.BREAK_REASON_REACHED_TARGET_MCYCLE)
    assert(machine:read_reg("mcycle") == MAX_MCYCLE)
    assert(machine:read_reg("iflags_H") == 0)
...
do_test("run_uarch should materialize overflow at max uarch_cycle", function(machine)
    machine:write_reg("uarch_cycle", MAX_UARCH_CYCLE)
    assert(machine:read_reg("uarch_cycle") == MAX_UARCH_CYCLE)
    assert(machine:run_uarch(MAX_UARCH_CYCLE) == cartesi.UARCH_BREAK_REASON_REACHED_TARGET_UARCH_CYCLE)
...
do_test("logged uarch step should materialize overflow at max uarch cycle", function(machine)
    machine:write_reg("uarch_cycle", MAX_UARCH_CYCLE)
    local root_hash_before = machine:get_root_hash()
    ...
    assert(log.accesses[1].address == cartesi.UARCH_SHADOW_START_ADDRESS) -- address of uarch_halt
    assert(log.accesses[2].type == "read")
    assert(log.accesses[2].address == cartesi.UARCH_SHADOW_START_ADDRESS + 8) -- address of uarch_cycle
    assert(#log.accesses[2].read == 32)
    -- log data has 32 bytes. The uarch_cycle is the 2nd 8-byte word
    assert(log.accesses[2].read:sub(9, 16) == string.pack("J", MAX_UARCH_CYCLE))
    assert(log.accesses[2].sibling_hashes ~= nil)
    assert(log.accesses[3].type == "write")
    assert(log.accesses[3].address == cartesi.UARCH_SHADOW_START_ADDRESS)
