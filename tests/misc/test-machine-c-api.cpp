...
// When CARTESI_JSONRPC_TEST_REMOTE is set to the cartesi-jsonrpc-machine binary,
// a single server is spawned once and all tests run against it over the network.
// The server can hold at most one machine, so every wrapper destroys any existing
// machine before creating or loading a new one.
static cm_machine *g_server = nullptr; // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)

struct remote_server_global_fixture {
    remote_server_global_fixture() {
        const char *server_bin = std::getenv("CARTESI_JSONRPC_TEST_REMOTE");
        if (server_bin == nullptr || server_bin[0] == '\0') {
            return;
        }
        // cm_jsonrpc_spawn_server() locates the binary via CARTESI_JSONRPC_MACHINE.
        // NOLINTNEXTLINE(cert-env33-c)
        setenv("CARTESI_JSONRPC_MACHINE", server_bin, 1);
        const char *bound = nullptr;
        if (cm_jsonrpc_spawn_server("127.0.0.1:0", 15000, &g_server, &bound, nullptr) != CM_ERROR_OK) {
            std::cerr << "WARNING: cm_jsonrpc_spawn_server: " << cm_get_last_error_message() << "\n";
            g_server = nullptr;
        }
    }
    ~remote_server_global_fixture() {
        ...
        cm_jsonrpc_set_cleanup_call(g_server, CM_JSONRPC_SHUTDOWN);
        cm_delete(g_server);
        g_server = nullptr;
    }
    remote_server_global_fixture(const remote_server_global_fixture &) = delete;
    remote_server_global_fixture(remote_server_global_fixture &&) = delete;
    remote_server_global_fixture &operator=(const remote_server_global_fixture &) = delete;
    remote_server_global_fixture &operator=(remote_server_global_fixture &&) = delete;
};
BOOST_GLOBAL_FIXTURE(remote_server_global_fixture);

namespace {

// Creates a machine: remotely on g_server when active, locally otherwise.
// Any existing machine on the server is destroyed first (the server holds at most one).
// Falls back to cm_create_new when out is null (local-only null-output tests).
cm_error test_create_new(const char *config, const char *runtime, const char *dir, cm_machine **out) {
    if (g_server != nullptr && out != nullptr) {
        cm_destroy(g_server); // defensive: evict any leftover machine
        *out = g_server;
        return cm_create(g_server, config, runtime, dir);
    }
    return cm_create_new(config, runtime, dir, out);
}

// Loads a machine from disk: remotely on g_server when active, locally otherwise.
// Falls back to cm_load_new when out is null (local-only null-output tests).
cm_error test_load_new(const char *dir, const char *runtime, cm_sharing_mode sharing, cm_machine **out) {
    if (g_server != nullptr && out != nullptr) {
        cm_destroy(g_server);
        *out = g_server;
        return cm_load(g_server, dir, runtime, sharing);
    }
    return cm_load_new(dir, runtime, sharing, out);
}

// Deletes a machine handle: for the shared server handle, destroys the machine
// but keeps the server alive; for a local handle, calls cm_delete as usual.
void test_delete(cm_machine **m) {
    if (*m == nullptr) {
        return;
    }
    if (*m == g_server) {
        cm_destroy(*m); // wipe machine state on server, keep handle alive
    } else {
        cm_delete(*m); // local handle: free it
    }
    *m = nullptr;
}
...
BOOST_AUTO_TEST_CASE_NOLINT(rollup_constants_test) {
    BOOST_CHECK_EQUAL(CM_ROLLUP_LOG2_MAX_MCYCLES_PER_ADVANCE_STATE, 48);
    BOOST_CHECK_EQUAL(CM_ROLLUP_LOG2_MAX_UARCH_CYCLES_PER_MCYCLE, 20);
    BOOST_CHECK_EQUAL(CM_ROLLUP_LOG2_MAX_OUTPUT_COUNT, 63);
    BOOST_CHECK_EQUAL(CM_ROLLUP_LOG2_MAX_ADVANCE_STATES_PER_EPOCH, 24);
    BOOST_CHECK_EQUAL(CM_UARCH_CYCLE_MAX, (UINT64_C(1) << CM_ROLLUP_LOG2_MAX_UARCH_CYCLES_PER_MCYCLE) - 1);
}
...
BOOST_FIXTURE_TEST_CASE_NOLINT(load_machine_unknown_dir_test, default_machine_fixture) {
    cm_error error_code = test_load_new("/unknown_dir", nullptr, CM_SHARING_NONE, &_machine);
...
BOOST_FIXTURE_TEST_CASE_NOLINT(load_machine_null_path_test, default_machine_fixture) {
    cm_error error_code = test_load_new(nullptr, nullptr, CM_SHARING_NONE, &_machine);
...
BOOST_FIXTURE_TEST_CASE_NOLINT(create_machine_null_config_test, default_machine_fixture) {
    cm_error error_code = test_create_new(nullptr, nullptr, nullptr, &_machine);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_INVALID_ARGUMENT);
    std::string result = cm_get_last_error_message();
...
BOOST_FIXTURE_TEST_CASE_NOLINT(create_machine_default_machine_test, default_machine_fixture) {
    cm_error error_code = test_create_new(_default_machine_config.c_str(), nullptr, nullptr, &_machine);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_INVALID_ARGUMENT);
...
BOOST_FIXTURE_TEST_CASE_NOLINT(create_machine_null_machine_test, incomplete_machine_fixture) {
    ...
    cm_error error_code = test_create_new(dumped_config.c_str(), nullptr, nullptr, nullptr);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_INVALID_ARGUMENT);
...
BOOST_FIXTURE_TEST_CASE_NOLINT(load_machine_invalid_config_version_test, serialized_machine_fixture) {
    ...
    cm_error error_code = test_load_new(_machine_config_path.c_str(), nullptr, CM_SHARING_NONE, &restored_machine);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_RUNTIME_ERROR);
    BOOST_CHECK_EQUAL(std::string(cm_get_last_error_message()), expected_err.str());

    test_delete(&restored_machine);
}
...
BOOST_FIXTURE_TEST_CASE_NOLINT(load_machine_null_machine_test, ordinary_machine_fixture) {
    ...
    error_code = test_load_new(_machine_dir_path.c_str(), nullptr, CM_SHARING_NONE, nullptr);
    BOOST_REQUIRE_EQUAL(error_code, CM_ERROR_INVALID_ARGUMENT);
}

BOOST_FIXTURE_TEST_CASE_NOLINT(serde_complex_test, ordinary_machine_fixture) {
    BOOST_REQUIRE_EQUAL(cm_write_reg(_machine, CM_REG_IMCYCLEMAX, 123), CM_ERROR_OK);
    ...
    cm_machine *restored_machine{};
    error_code = test_load_new(_machine_dir_path.c_str(), nullptr, CM_SHARING_NONE, &restored_machine);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
    BOOST_CHECK_EQUAL(std::string(""), std::string(cm_get_last_error_message()));
    ...
    uint64_t restored_imcyclemax{};
    BOOST_REQUIRE_EQUAL(cm_read_reg(restored_machine, CM_REG_IMCYCLEMAX, &restored_imcyclemax), CM_ERROR_OK);
    BOOST_CHECK_EQUAL(restored_imcyclemax, 123);
...
BOOST_FIXTURE_TEST_CASE_NOLINT(imcyclemax_read_write_test, ordinary_machine_fixture) {
    uint64_t imcyclemax{};
    BOOST_CHECK_EQUAL(cm_read_reg(_machine, CM_REG_IMCYCLEMAX, &imcyclemax), CM_ERROR_OK);
    BOOST_CHECK_EQUAL(imcyclemax, UINT64_MAX);
    BOOST_CHECK_EQUAL(cm_write_reg(_machine, CM_REG_IMCYCLEMAX, 123), CM_ERROR_OK);
    BOOST_CHECK_EQUAL(cm_read_reg(_machine, CM_REG_IMCYCLEMAX, &imcyclemax), CM_ERROR_OK);
    BOOST_CHECK_EQUAL(imcyclemax, 123);
}
...
BOOST_FIXTURE_TEST_CASE_NOLINT(verify_step_uarch_null_log_test, default_machine_fixture) {
    cm_error error_code = cm_verify_step_uarch(nullptr, nullptr, nullptr, 0, 0, nullptr);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_INVALID_ARGUMENT);

    std::string result = cm_get_last_error_message();
    std::string origin("invalid log");
    BOOST_CHECK_EQUAL(origin, result);
}
...
public:
    ...
    step_log_machine_fixture(const step_log_machine_fixture &other) = delete;
    step_log_machine_fixture(step_log_machine_fixture &&other) noexcept = delete;
    step_log_machine_fixture &operator=(const step_log_machine_fixture &other) = delete;
    step_log_machine_fixture &operator=(step_log_machine_fixture &&other) noexcept = delete;

protected:
    // Logs one uarch step and copies the log out of the C API's temporary storage, which
    // only lasts until the next C API call.
    std::vector<uint8_t> log_step_uarch(cm_error &error_code) {
        const uint8_t *log{};
        uint64_t log_length{};
        error_code = cm_log_step_uarch(_machine, 1, &log, &log_length, nullptr);
        if (error_code != CM_ERROR_OK) {
            return {};
        }
        return {log, log + log_length};
    }
...
BOOST_FIXTURE_TEST_CASE_NOLINT(step_null_machine_test, step_log_machine_fixture) {
    const uint8_t *log{};
    uint64_t log_length{};
    cm_error error_code = cm_log_step_uarch(nullptr, 1, &log, &log_length, nullptr);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_INVALID_ARGUMENT);
}

BOOST_FIXTURE_TEST_CASE_NOLINT(step_null_log_output_test, step_log_machine_fixture) {
    cm_error error_code = cm_log_step_uarch(_machine, 1, nullptr, nullptr, nullptr);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_INVALID_ARGUMENT);

    std::string result = cm_get_last_error_message();
    std::string origin("invalid log output");
    BOOST_CHECK_EQUAL(origin, result);
}

BOOST_FIXTURE_TEST_CASE_NOLINT(verify_step_uarch_null_hash0_test, step_log_machine_fixture) {
    cm_error error_code{};
    ...
    cm_hash hash1;
    error_code = cm_verify_step_uarch(nullptr, nullptr, log.data(), log.size(), 1, &hash1);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_INVALID_ARGUMENT);
...
BOOST_FIXTURE_TEST_CASE_NOLINT(verify_step_uarch_obtained_hash_test, step_log_machine_fixture) {
    ...
    // the obtained hash is returned for the caller to check
    cm_hash obtained_root_hash;
    error_code = cm_verify_step_uarch(nullptr, &hash0, log.data(), log.size(), 1, &obtained_root_hash);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
    BOOST_CHECK_EQUAL(0, memcmp(obtained_root_hash, hash1, sizeof(cm_hash)));
}

BOOST_FIXTURE_TEST_CASE_NOLINT(verify_step_uarch_null_log_with_hashes_test, step_log_machine_fixture) {
    cm_hash hash0;
    cm_hash hash1;
    cm_error error_code = cm_verify_step_uarch(nullptr, &hash0, nullptr, 0, 0, &hash1);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_INVALID_ARGUMENT);
...
BOOST_FIXTURE_TEST_CASE_NOLINT(log_step_uarch_until_halt, step_log_machine_fixture) {
    cm_hash hash0{};
    cm_hash hash1{};
    cm_hash hash2{};
    cm_hash hash3{};
    ...
    // not halted
    error_code = cm_read_reg(_machine, CM_REG_UARCH_HALT, &halt);
    BOOST_REQUIRE_EQUAL(error_code, CM_ERROR_OK);
    ...
    // step 1
    const auto log1 = log_step_uarch(error_code);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
    error_code = cm_get_root_hash(_machine, &hash1);
    BOOST_REQUIRE_EQUAL(error_code, CM_ERROR_OK);
    error_code = cm_verify_step_uarch(nullptr, &hash0, log1.data(), log1.size(), 1, &obtained);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
    BOOST_CHECK_EQUAL(0, memcmp(obtained, hash1, sizeof(cm_hash)));

    // step 2
    const auto log2 = log_step_uarch(error_code);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
    error_code = cm_get_root_hash(_machine, &hash2);
    BOOST_REQUIRE_EQUAL(error_code, CM_ERROR_OK);
    error_code = cm_verify_step_uarch(nullptr, &hash1, log2.data(), log2.size(), 1, &obtained);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
    BOOST_CHECK_EQUAL(0, memcmp(obtained, hash2, sizeof(cm_hash)));

    // step 3
    const auto log3 = log_step_uarch(error_code);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
    error_code = cm_get_root_hash(_machine, &hash3);
    BOOST_REQUIRE_EQUAL(error_code, CM_ERROR_OK);
    error_code = cm_verify_step_uarch(nullptr, &hash2, log3.data(), log3.size(), 1, &obtained);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
    BOOST_CHECK_EQUAL(0, memcmp(obtained, hash3, sizeof(cm_hash)));

    // step 4
    const auto log4 = log_step_uarch(error_code);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
    error_code = cm_get_root_hash(_machine, &hash4);
    BOOST_REQUIRE_EQUAL(error_code, CM_ERROR_OK);
    error_code = cm_verify_step_uarch(_machine, &hash3, log4.data(), log4.size(), 1, &obtained);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
...
BOOST_FIXTURE_TEST_CASE_NOLINT(step_complex_test, step_log_machine_fixture) {
    cm_hash hash0;
    cm_hash hash1;
    ...
    cm_hash obtained_root_hash{};
    error_code = cm_verify_step_uarch(_machine, &hash0, log.data(), log.size(), 1, &obtained_root_hash);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
    BOOST_CHECK_EQUAL(std::string(""), std::string(cm_get_last_error_message()));
    BOOST_CHECK_EQUAL_COLLECTIONS(obtained_root_hash, obtained_root_hash + sizeof(cm_hash), hash1,
        hash1 + sizeof(cm_hash));
}

BOOST_FIXTURE_TEST_CASE_NOLINT(step_hash_test, step_log_machine_fixture) {
    cm_error error_code{};
    log_step_uarch(error_code);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
    BOOST_CHECK_EQUAL(std::string(""), std::string(cm_get_last_error_message()));
...
BOOST_AUTO_TEST_CASE_NOLINT(machine_run_uarch_null_machine_test) {
    auto status{CM_UARCH_BREAK_REASON_REACHED_TARGET_UARCH_CYCLE};
    cm_error error_code = cm_run_uarch(nullptr, 1000, &status);
    BOOST_REQUIRE_EQUAL(error_code, CM_ERROR_INVALID_ARGUMENT);
}

BOOST_FIXTURE_TEST_CASE_NOLINT(machine_run_uarch_advance_one_cycle, step_log_machine_fixture) {
    ...
    BOOST_REQUIRE_EQUAL(error_code, CM_ERROR_OK);
    BOOST_REQUIRE_EQUAL(std::string(cm_get_last_error_message()), std::string(""));
    BOOST_REQUIRE_EQUAL(status, CM_UARCH_BREAK_REASON_REACHED_TARGET_UARCH_CYCLE);
...
BOOST_FIXTURE_TEST_CASE_NOLINT(machine_run_uarch_advance_until_halt, step_log_machine_fixture) {
    // ensure that uarch cycle is 0
    uint64_t cycle{};
    cm_error error_code = cm_read_reg(_machine, CM_REG_UARCH_CYCLE, &cycle);
    BOOST_REQUIRE_EQUAL(error_code, CM_ERROR_OK);
