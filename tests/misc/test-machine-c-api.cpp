...
public:
    grpc_machine_fixture() : machine_flash_simple_fixture() {
        char *err_msg{};
        int result = cm_create_grpc_machine_stub("127.0.0.1:7001", "127.0.0.1:7002", &m_stub, &err_msg);
...
public:
    grpc_machine_fixture_with_server() {
        auto process_path = std::filesystem::current_path() / "../src/remote-cartesi-machine";
        if (!std::filesystem::exists(process_path)) {
            process_path = "/usr/bin/remote-cartesi-machine";
        }
        boost::process::spawn(process_path.string(), "127.0.0.1:7001", m_server_group);
...
BOOST_AUTO_TEST_CASE_NOLINT(create_grpc_machine_stub_no_server_test) {
    char *err_msg{};
    cm_grpc_machine_stub *stub{};
    int error_code = cm_create_grpc_machine_stub("127.0.0.2:7001", "127.0.0.1:7002", &stub, &err_msg);
    BOOST_CHECK_EQUAL(error_code, CM_ERROR_OK);
    BOOST_REQUIRE_EQUAL(err_msg, nullptr);
