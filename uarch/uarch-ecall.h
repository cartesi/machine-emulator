...
// NOLINTBEGIN(hicpp-no-assembler): the ecall and its register pinning are the point

static inline void ua_halt_ECALL() {
    register const uint64_t a7 __asm__("a7") = UARCH_ECALL_FN_HALT_DEF;
    __asm__ volatile("ecall" : : "r"(a7) : "memory");
}

static inline void ua_putchar_ECALL(uint8_t c) {
    register const uint64_t a7 __asm__("a7") = UARCH_ECALL_FN_PUTCHAR_DEF;
    register const uint64_t a0 __asm__("a0") = c;
...
static inline void ua_write_tlb_ECALL(uint64_t use, uint64_t slot_index, uint64_t vaddr_page, uint64_t vp_offset,
    uint64_t pma_index) {
    register const uint64_t a7 __asm__("a7") = UARCH_ECALL_FN_WRITE_TLB_DEF;
    register const uint64_t a0 __asm__("a0") = use;
    register const uint64_t a1 __asm__("a1") = slot_index;
    register const uint64_t a2 __asm__("a2") = vaddr_page;
    register const uint64_t a3 __asm__("a3") = vp_offset;
    register const uint64_t a4 __asm__("a4") = pma_index;
