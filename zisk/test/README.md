...
Runs the ZisK prover against step logs

## Usage

Run all tests with ziskemu (fast).
Public output values written to the output dir.
...
The test target writes to outputs/<test-name>/:

  public_output_values.bin      - Raw public outputs from ziskemu (72 bytes)

The prove target adds to outputs/<test-name>/:
...
The result.json contains:
  ...
  cycles  - Total ZisK execution cycles
  time    - Proof generation time in seconds
  id      - Proof identifier hash
...
Run ziskemu with -c to see these values printed to console.
